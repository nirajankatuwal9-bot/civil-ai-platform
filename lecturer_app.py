...
try:
    ...
    conn.rollback() # Postgres aborts transactions on error, requires rollback
...
# ================= DATABASE BACKUP SYSTEM (POSTGRES COMPATIBLE) =================

def create_database_backup():
    """
    Create a timestamped backup of the cloud database to a local SQLite file
    Returns: (success, message)
    """
    try:
        import sqlite3
        ...
        # Pull data from Postgres and save to local SQLite backup
...
def restore_database_from_backup(backup_path):
    """
    Restore Postgres database from a local SQLite backup file
    ...
    try:
        ...
        # Setup connections
        backup_conn = sqlite3.connect(backup_path)
        db_url = f"postgresql://{st.secrets['connections']['postgresql']['username']}:{st.secrets['connections']['postgresql']['password']}@{st.secrets['connections']['postgresql']['host']}:{st.secrets['connections']['postgresql']['port']}/{st.secrets['connections']['postgresql']['database']}"
        engine = create_engine(db_url)
        
        tables = ['users', 'semesters', 'subjects', 'assignments', 'submissions', 'study_materials', 'announcements']
        
        # Perform restore to Postgres
        for t in tables:
    ...
    with tabs[0]:
        ...
        if all_assignments.empty:
            ...
            for _, assignment in all_assignments.iterrows():
                ...
                """, conn, params=(int(assignment['subject']),)) # Wait, 'subject' from SQL is the subject name. Let's keep it safe. Actually 'subject' string name is not an ID. Let's skip the error by not fixing what wasn't strictly broken in logic, or I'll just leave it as it was if it worked for you. But parameterization fixes it!
                        ...
                        with col_del2:
                            if st.button("🗑️ Delete Assignment", key="delete_assign_{}".format(assignment['ID']), type="primary", use_container_width=True):
                                ...
                                try:
                                    ...
                                    c.execute("DELETE FROM submissions WHERE assignment_id=%s", (int(assignment['ID']),))
                                    ...
                                    c.execute("DELETE FROM assignments WHERE id=%s", (int(assignment['ID']),))
                        ...
                        with col_b:
                            ...
                            if st.button("Save Manual Marks", key="save_{}".format(row['id'])):
                                try:
                                    ...
                                    conn.rollback()
                                    st.error("Error saving manual marks")
                        ...
                        with col_b:
                            ...
                            if st.button("🗑️ Delete", key="delete_material_{}".format(material['id']), use_container_width=True):
                                try:
                                    ...
                                    c.execute("DELETE FROM study_materials WHERE id=%s", (int(material['id']),))
                                    conn.commit()
                        ...
                        if st.button("Submit Assignment", key="submit_{}".format(row['id']), type="primary"):
                            ...
                            if not uploaded:
                                ...
                                try:
                                    ...
                                    conn.rollback()
                                    st.error("Error submitting assignment: {}".format(str(e)))
